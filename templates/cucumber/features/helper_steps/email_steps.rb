...
Then(/^(?:I|they) should not see "([^"]*?)" in the email subject$/) do |text|
  expect(current_email).to_not have_subject(text)
end

Then(/^(?:I|they) should not see \/([^"]*?)\/ in the email subject$/) do |text|
  expect(current_email).to_not have_subject(Regexp.new(text))
end
...
Then(/^(?:I|they) should not see "([^"]*?)" in the email body$/) do |text|
  expect(current_email.default_part_body.to_s).to_not include(text)
end
...
Then(/^(?:I|they) should not see \/([^"]*?)\/ in the email body$/) do |text|
  expect(current_email.default_part_body.to_s).to_not match Regexp.new(text)
end
